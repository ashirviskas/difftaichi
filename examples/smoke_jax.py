...
def project(vx, vy):
    ...
    h = 1.0 / vx.shape[0]
    div = -0.5 * h * (
            np.roll(vx, -1, axis=0) - np.roll(vx, 1, axis=0) + np.roll(
        vy, -1, axis=1) - np.roll(vy, 1, axis=1))

    # Note: for some reason JAX crashes when num_iteration != 5/6.
    for k in range(6):
        p = (div + np.roll(p, 1, axis=0) + np.roll(p, -1, axis=0) + np.roll(
            p, 1, axis=1) + np.roll(p, -1, axis=1)) / 4.0
